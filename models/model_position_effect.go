...
func (v *PositionEffect) UnmarshalJSON(src []byte) error {
	...
	for _, existing := range AllowedPositionEffectEnumValues {
...
// NewPositionEffectFromValue returns a pointer to a valid PositionEffect
...
func NewPositionEffectFromValue(v string) (*PositionEffect, error) {
	...
	if ev.IsValid() {
		...
		return nil, fmt.Errorf("invalid value '%v' for PositionEffect: valid values are %v", v, AllowedPositionEffectEnumValues)
	}
}
